...
trait Solver extends GameDef {
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): Stream[(Block, List[Move])] = {
    b.legalNeighbors.map(legalNeighbor=>
      (legalNeighbor._1, legalNeighbor._2 +: history)
    ).toStream
  }
  /**
  ...
  lazy val pathsFromStart: Stream[(Block, List[Move])] = {
    from((startBlock, List()) #:: Stream[(Block, List[Move])](), Set())
  }
   ...
   */
  lazy val pathsToGoal: Stream[(Block, List[Move])] = pathsFromStart.filter(f=> done(f._1))
